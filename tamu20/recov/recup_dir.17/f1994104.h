 />
		...
		<script type="text/jscript" defer>
			...
			{
				sConnTestURL = unescape( sConnTestURL );
				if( sConnTestURL == ValidateURL(sConnTestURL) )
				{
				...
				{
					location.href = "hcp://system/errors/badurl.htm";
				}
			}
			...
			function OpenURL( bServerStatusOK )
			{
				...
				if( false == bServerStatusOK )
				{
					...
					{
						ASSERT( "setTimeout( 'CreateConnCheck( \"" + escape(g_sTargetURL) + "\" )', 100 )" );
					}
				}
			...
			function window::onload()
			{
				...
				g_sTopicTitle = oPCH_TxtHlp.HTMLEscape(oParsedURL.GetQueryParameter( "topic_title" ));
				g_sTopicIntroBlurb = oPCH_TxtHlp.HTMLEscape(oParsedURL.GetQueryParameter( "topic_intro" ));
