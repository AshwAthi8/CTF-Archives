ype.toLowerCase() == "lan")
    {
        divConnectHelp.style.display = "";
    }

    if(oLM.LogonStatus == 0)
    {
        if(oConnectors.Default.DeviceType.toLowerCase() == "lan")
        {
            oMarsSignup.ConnectForSignup();
        }
        else
        {
            MarsLog("Init() -- Attempted to launch signup page via. Modem with no connectivity!", true);
        }
